...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val hotZoneAnalysis = spark.sql("select rectangle,count(point) as count from joinResult group by rectangle order by rectangle asc")
    hotZoneAnalysis.createOrReplaceTempView("hotZone")

    return hotZoneAnalysis
  }
